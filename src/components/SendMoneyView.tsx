...
export const SendMoneyView: React.FC<SendMoneyViewProps> = ({
  ...
  const handleExecuteVerifiedTransfer = () => {
    if (transferMode === 'P2P' && selectedRecipient) {
      ...
      // Call backend API endpoint to register payout and trigger switch webhook log
      fetch('/api/transfers/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          senderEmail: user.email,
          recipientName: resolvedAccountName || 'Account Holder',
          accountNumber,
          bankName: selectedBank.name,
          amount: numPayoutSend,
          currency: payoutDebitCurrency,
        }),
