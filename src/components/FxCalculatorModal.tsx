...
export const FxCalculatorModal: React.FC<FxCalculatorModalProps> = ({ isOpen, onClose }) => {
  ...
  const [liveRates, setLiveRates] = useState<Record<string, number>>(FX_RATES_TO_USD);
  ...
  const fromToUsd = 1 / (liveRates[fromCurrency] || FX_RATES_TO_USD[fromCurrency] || 1);
  const usdToTarget = liveRates[toCurrency] || FX_RATES_TO_USD[toCurrency] || 1;
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex items-center justify-between border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Zero markup mid-market exchange rates</p>
            </div>
          </div>
