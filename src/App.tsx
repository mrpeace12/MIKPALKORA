...
export default function App() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      if (err.message && (err.message.includes('Unauthorized') || err.message.includes('Invalid'))) { api.clearToken(); setIsAuthenticated(false); }
    } fontually: { setIsLoading(false); }
  }, []);
  ...
  const handleExecuteBankPayout = async (sendAmount: number, debitCurrency: string, destBank: BankDestination, accountNumber: string, accountName: string) => {
    try {
      const pin = prompt('Enter your 4-digit PIN:'); if (!pin) return;
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={handleSignOut} className="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm font-bold cursor-pointer">Sign Out</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-20 md:pb-0">
        <Header
          user={activeUser}
          onMenuClick={() => setShowSideDrawer(true)}
        />
        <main className="flex-1 overflow-y-auto p-4 md:p-6 max-w-6xl mx-auto w-full">
          ...
          {activeTab === 'SEND' && (
            <SendMoneyView
              user={activeUser}
              onP2PTransfer={handleExecuteP2PTransfer}
              onBankPayout={handleExecuteBankPayout}
            />
          )}
          {activeTab === 'CARDS' && (
            <VirtualCardsView />
          ...
          {activeTab === 'PROFILE' && (
            <ProfileView user={activeUser} onUpdate={handleUpdateUserProfile} onSignOut={handleSignOut} />
          )}
        </main>
      </div>
      <BottomNav activeTab={activeTab} onTabChange={(tab) => setActiveTab(tab)} onDeposit={() => setShowDepositModal(true)} user={activeUser} />
